...
\begin{document}
	...
	\begin{enumerate}
		...
		\pagebreak
		\item \textbf{Spatial Non-linear Filtering} \textit{(5x5 median filter)}
		\begin{enumerate}
			\begin{multicols}{2}
				\item Added gaussian noise \\
			\includegraphics[scale=0.75]{task2/img_gaussian.png}
			
			\item Added salt and pepper noise \\
			\includegraphics[scale=0.75]{task2/img_sp.png}
						
			\item Median filter on (a) \\
			\includegraphics[scale=0.75]{task2/med_gaussian.png}
			
			\item Median filter on (b) \\
		...
		\pagebreak
		\item \textbf{High-Frequency Emphasis} \textit{(a = 0.5, b = 2.0)}
		\begin{enumerate}
			\begin{multicols}{2}
			\item Butterworth \\
